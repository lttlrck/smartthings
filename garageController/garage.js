...
var push = new Pushover(JSON.parse( fs.readFileSync("pushover.json")));
var wukey = fs.readFileSync("wu.json").key;
...
(function checkSensors()
{
    ...
    setTimeout( function()
    {
        ...
        if(( status !== bits) || (counter === 15))
        {
            notify("graph.api.smartthings.com", 443, "/api/smartapps/installations/"+installationId+"/event/update", function(){ console.log("Status Change:"+bits)})
...
// http://api.wunderground.com/api/5ad2f86a0d64ad8e/forecast/q/MA/Chelmsford.json
...
(function check() {
    ...
    http.get({
            ...
            path: '/api/'+wukey+'/forecast/q/MA/Chelmsford.json'
        }, function(response) {
            ...
            response.on('end', function() {
                ...
                var today= parsed.forecast.simpleforecast.forecastday[0];

                var reason;

                if( today.qpf_day.mm < 3)
                {
                    console.log( "dry");

                    // pfio.digital_write(2,1); // on if no rain

                    var temp= " ("+today.high.celsius+"°C)"

                    if( +today.high.celsius >= 25) {

                        sprinkle= true;

                        reason= "dry and warm"+temp;
                    }
                    else
                    {
                        reason= "dry but cool"+temp;
                    }
                }
                else
                {
                    reason= "will rain "+today.qpf_allday.mm+"mm";

                    console.log( "Rain", today.qpf_allday.mm+"mm");

                    // pfio.digital_write(2,0); // off if rain
                ...
                if( sprinkleValue !== sprinkle)
                {
                    push.send("Sprinkler", (sprinkle?"on":"off") + ", " + reason);

                    sprinkleValue= sprinkle;
                }
            });
        });

    setTimeout( check, 60000*15);
    // setTimeout( check, 3600000);
})();

// push.send("Sprinkler","Reboot");
